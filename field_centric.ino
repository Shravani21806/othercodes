...
int solenoid_dribbling = 42;  // Pin 2 of Arduino connected to Pin 1 (IN1) of ULN2003
int solenoid_angle = 43;
int solenoid_base = 40;
int solenoid_net = 41;
...
void loop() {
  ...
  //   Serial.println(fieldCentric ? "ON" : "OFF");
  // }
  // if (L1 == 0 && L1_ref == 1) {
  //   L1_ref = 0;
  // }
  ...
  if (L1==1 && select_ref==0) {
    field=!field;
    select_ref=1;
  }
  if(L1==0 && select_ref==1){
    select_ref=0;
  }

  if(field){
    theta=theta-yaw;
    Serial.print("in field centric");
    }
